...
parser.add_argument('--save_path', type=str, default="/mnt/disk1/users/afshari/MyEcapaModelTry2Ver2", help='Path to save the score.txt and models')
parser.add_argument('--initial_model', type=str, default="", help='Path of the initial_model')
parser.add_argument('--path_save_model', type=str, default="/mnt/disk1/users/afshari/MyEnrollmentTry2Ver2", help='Path to save the enrollment and models')
...
# Enroll data loader
enrollloader = enroll_loader(file_list=args.enroll_list, data_path=args.enroll_path, num_frames=args.num_frames)
enrollLoader = torch.utils.data.DataLoader(enrollloader, batch_size=1, shuffle=False, num_workers=args.n_cpu, drop_last=False)

# Test data loader
testloader = test_loader(file_list=args.eval_list, data_path=args.eval_path, num_frames=args.num_frames)
...
while(1):
    ...
    if epoch > 5: # I should change it later if I want to train from the base ........5.6.5024
        loss, lr, acc = s.train_network(epoch=epoch, loader=trainLoader)

    # Enrollment and Testing every [test_step] epochs
    if epoch % args.test_step == 0:
        s.save_parameters(args.save_path + "/model_%04d.model" % epoch)
        s.enroll_network(enrollLoader=enrollLoader, path_save_model=args.path_save_model)
        
        # Test with evaluation trials (without trial_type)
        #EER, minDCF, scores = s.test_network(testLoader=testLoader, path_save_model=args.path_save_model, compute_eer=False)
        
        # Test with development trials (with trial_type)
        dev_EER, dev_minDCF, dev_scores = s.test_network(testLoader=testLoader, path_save_model=args.path_save_model, compute_eer=True)

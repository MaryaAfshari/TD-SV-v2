...
class ECAPAModel(nn.Module):
    ...
    def enroll_network(self, enroll_list, enroll_path, path_save_model):
        ...
        # Pad audio data to the same length
        for i in range(len(all_audio_data)):
            audio = all_audio_data[i]
            if audio.shape[1] < max_length:
                padding = torch.zeros((audio.shape[0], max_length - audio.shape[1])).cuda()
                all_audio_data[i] = torch.cat((audio, padding), dim=1)
    ...
    def test_network(self, test_list, test_path, path_save_model, compute_eer=True):
        ...
        for i in range(len(audio_data)):
            ...
            if audio.shape[1] < max_length:
                padding = torch.zeros((audio.shape[0], max_length - audio.shape[1])).cuda()
                audio_data[i] = torch.cat((audio, padding), dim=1)

...
class ECAPAModel(nn.Module):
    ...
    def enroll_network(self, enrollLoader, path_save_model):
        ...
        for data, label in enrollLoader:
            ...
            audio = data.cuda()
            with torch.no_grad():
                embedding = self.speaker_encoder.forward(audio, aug=False)
                embedding = F.normalize(embedding, p=2, dim=1)
            if model_id not in enrollments:
                enrollments[model_id] = []
            enrollments[model_id].append(embedding)

        for model_id in enrollments:
            enrollments[model_id] = torch.mean(torch.stack(enrollments[model_id]), dim=0)
    ...
    def test_network(self, testLoader, path_save_model, compute_eer=True):
        ...
        for data, label in testLoader:
            ...
            score = torch.mean(torch.matmul(test_embedding, enrollments[test_file].T)).detach().cpu().numpy()
            scores.append(score)
            if len(label) > 1:  # Assuming label contains more info if available
